...
function renderMimeIcon(mimeType?: string) {
  if (!mimeType) return <FileIcon className="size-5" />;
  const type = mimeType.toLowerCase();
  if (type.startsWith("image/")) return <ImageIcon className="size-5" />;
  if (type === "application/pdf") return <FileTextIcon className="size-5" />;
  if (type === "application/json") return <BracesIcon className="size-5" />;
  if (type.startsWith("text/")) return <FileTextIcon className="size-5" />;
...
function FileIconDisplay({
  ...
  return (
    <span
      ...
      {...props}
    >
      {children ?? renderMimeIcon(mimeType)}
    </span>
  );
}
...
export {
  ...
  fileVariants,
  renderMimeIcon,
  renderMimeIcon as getMimeTypeIcon,
  getFileDataKind,
  getBase64Size,
  formatFileSize,
};
